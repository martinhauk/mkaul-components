 ...
 * @author Manfred Kaul <manfred.kaul@h-brs.de> 2017, 2018, 2019, 2020, 2021, 2022
 ...
 ( function () {
  ...
  var component  = {
    ...
    config: {
      ...
      open_to: 24,
      delay: 100,
      // simulation_start_date: '2022-03-27T20:00:00',
      // simulation_speed: 10,
      simulation: {
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        self.instanceStartDate = new Date;
        ...
        class Car {
          ...
          price() {
            ...
            return Math.round(this.price_factor() * this._duration / parseInt( self.simulation_speed || 1 ) );
          }
          color(){
            if ( ! this._color ){
          ...
          toArray(){
            const list = [ this.nr, this.timer, this._duration, this.price(), this.hash(), this.color(), this.space, this.client_category, this.vehicle_type, this.license, this.beginTime(), self.name ];
            // if ( this.timer !== this.beginTime() ) list.push( this.beginTime() );
            return list.map( x => ( x || '_' ).toString() );
          }
          toCSV(){
            this.toArray().join(',');
          }
        ...
        function simulationTime( realTime ){
          const startDate = new Date( self.simulation_start_date || self.instanceStartDate );
          const result = BigInt( startDate ) + ( BigInt( self.simulation_speed || 1 ) * ( BigInt( realTime ) - BigInt( startDate ) ) );
